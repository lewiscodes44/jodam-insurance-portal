...
export function CustomerDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getMyPolicies(), getMyQuotations(), getMyInquiries(), getMyClaims(), getMyNotifications()])
      .then(([p, q, i, c, n]) => { setPolicies(p); setQuotations(q); setInquiries(i); setClaims(c); setNotifications(n); setError('') })
  ...
  const updateMessage = (message:string) => { const policy=policies.find(p=>message.includes(p.policyNumber)); if(!policy)return message; const vehicle=vehicleTitle(inquiryById(inquiries,policy.inquiryId),policy.insuranceType); return message.replace(`Your policy ${policy.policyNumber}`,`${vehicle.registration} · ${vehicle.makeModel} (${vehicle.cover})`).replace(/Amount due: KES [\d,.]+/i,`Amount due: KES ${Number(policy.premiumAmount).toLocaleString()}`) }
  const unread = notifications.filter(n => !n.readAt).length
  const activities=[...inquiries.filter(i=>!policies.some(p=>p.inquiryId===i.id)).map(i=>({kind:'Inquiry',id:i.id,to:`/app/enquiries/${i.id}`,title:vehicleTitle(i).registration+' · '+vehicleTitle(i).makeModel,detail:vehicleTitle(i).cover,status:i.status})),...policies.map(p=>{const v=vehicleTitle(inquiryById(inquiries,p.inquiryId),p.insuranceType);return{kind:'Policy',id:p.id,to:`/app/policies/${p.id}`,title:v.registration+' · '+v.makeModel,detail:v.cover,status:p.status}}),...activeQuotations.map(q=>{const v=vehicleTitle(inquiryById(inquiries,q.inquiryId));return{kind:'Quotation',id:q.id,to:`/app/quotations/${q.id}`,title:v.registration+' · '+v.makeModel,detail:v.cover,status:q.status}}),...claims.map(c=>({kind:'Claim',id:c.id,to:`/app/claims/${c.id}`,title:c.claimNumber,detail:c.policyNumber,status:c.status}))]

  return <AppShell>
    <div className="page-heading"><div><span className="eyebrow eyebrow--red">Customer portal</span><h1>Your insurance, in one place.</h1><p>Track your cover, quotations, claims and payment activity.</p><div className="dashboard-quick-actions"><Link className="button button--primary" to="/app/enquiries/new"><Icon name="plus" /> New inquiry</Link><Link className="button button--primary" to="/app/claims/new"><Icon name="plus" /> New claim</Link></div></div></div>
