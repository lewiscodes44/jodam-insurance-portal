...
export function CustomerDashboardPage() {
  ...
  return <AppShell>
    ...
    {loading ? <div className="loading-state">Loading your portal…</div> : <>
      <section className="stat-grid"><div className="stat-card"><span>Active policies</span><strong>{activePolicies.length}</strong><small>Currently in force</small></div><div className="stat-card"><span>Quotations</span><strong>{quotations.length}</strong><small>Available to review</small></div><div className="stat-card"><span>Open claims</span><strong>{claims.filter(c => !['CLOSED','REJECTED'].includes(c.status)).length}</strong><small>Across your policies</small></div><div className="stat-card"><span>Notifications</span><strong>{unread}</strong><small>Need your attention</small></div></section>
      ...
      <section className="dashboard-grid"><div className="panel panel--wide"><div className="panel__header"><div><span className="eyebrow">Policies</span><h2>Your cover</h2></div><Link className="text-link" to="/app/policies">View all <Icon name="arrow" /></Link></div><div className="policy-list">{policies.slice(0,3).map(policy => <Link className="policy-row" key={policy.id} to={`/app/policies/${policy.id}`}><div className="policy-icon"><Icon name="car" /></div><div className="policy-main"><strong>{policy.insuranceType}</strong><span>{policy.policyNumber}</span></div><div className="policy-meta"><span>KES {Number(policy.premiumAmount).toLocaleString()}</span><StatusPill value={policy.status} /></div></Link>)}{policies.length===0 && <div className="empty-state empty-state--action"><p>No policies yet.</p><Link className="button button--secondary" to="/app/enquiries/new">Start a motor enquiry</Link></div>}</div></div>
      <div className="panel"><div className="panel__header"><div><span className="eyebrow">Recent activity</span><h2>Enquiries</h2></div><Link className="text-link" to="/app/enquiries">View all <Icon name="arrow" /></Link></div><div className="activity-list">{inquiries.slice(0,4).map(item => <Link className="activity-row" key={item.id} to={`/app/enquiries/${item.id}`}><div><strong>{item.insuranceType}</strong><span>{item.description}</span></div><StatusPill value={item.status} /></Link>)}{inquiries.length===0 && <p className="empty-state">No enquiries yet.</p>}</div></div></section>
      <section className="dashboard-grid dashboard-grid--lower"><div className="panel"><div className="panel__header"><div><span className="eyebrow">Quotations</span><h2>Ready to review</h2></div><Link className="text-link" to="/app/quotations">View all <Icon name="arrow" /></Link></div>{quotations.slice(0,3).map(q => <div className="quote-card" key={q.id}><div><strong>KES {Number(q.premiumAmount).toLocaleString()}</strong><span>Valid until {q.validUntil}</span></div><Link className="button button--secondary" to={`/app/quotations/${q.id}`}>Review</Link></div>)}{quotations.length===0 && <div className="empty-state empty-state--action"><p>No quotations to review.</p><Link className="text-link" to="/app/enquiries/new">Start an enquiry <Icon name="arrow" /></Link></div>}</div><div className="panel"><div className="panel__header"><div><span className="eyebrow">Notifications</span><h2>Latest updates</h2></div><Link className="text-link" to="/app/notifications">All <Icon name="arrow" /></Link></div>{notifications.slice(0,3).map(n => <div className="notification-row" key={n.id}><div className="notification-dot" /><div><strong>{n.subject ?? 'Jodam update'}</strong><p>{n.message}</p></div></div>)}{notifications.length===0 && <p className="empty-state">You're all caught up.</p>}</div></section>
    </>}
  </AppShell>
}
